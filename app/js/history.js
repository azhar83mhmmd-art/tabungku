...
const HistoryModule = (() => {
  ...
  function renderSummary(list){
    ...
    list.forEach(t => {
      if (t.type === 'income' || t.type === 'saving_out') income += t.amount;
      else if (t.type === 'expense' || t.type === 'saving_in') expense += t.amount;
    });
      ...
      const itemsHtml = g.items.map(t => {
        ...
        return `
          <div class="tx-item" data-tx-id="${t.id}" data-tx-type="${t.type}">
            ...
            <div class="tx-amount ${meta.sign === '+' ? 'in' : 'out'}">${meta.sign}${Utils.formatRupiah(t.amount)}</div>
