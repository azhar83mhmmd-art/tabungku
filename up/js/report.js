...
const ReportModule = (() => {
  ...
  function init(){
    ...
    const scanCsvBtn = document.getElementById('exportScanCsvBtn');
    const scanPdfBtn = document.getElementById('exportScanPdfBtn');
    if (scanCsvBtn) scanCsvBtn.addEventListener('click', exportScanCsv);
    if (scanPdfBtn) scanPdfBtn.addEventListener('click', exportScanPdf);
  }

  function exportScanCsv(){
    const history = (typeof ProductDB !== 'undefined') ? ProductDB.getScanHistory() : [];
    if (!history.length){ Utils.toast('Belum ada riwayat scan barcode', 'error'); return; }
    const header = ['Barcode','Nama Produk','Harga','Jumlah','Total','Waktu Scan','Sumber Data'];
    const rows = history.map(h => [
      h.barcode, h.name, h.price || 0, h.qty || 0, h.total || 0,
      new Date(h.scannedAt).toLocaleString('id-ID'), h.source || ''
    ]);
    const csv = [header, ...rows].map(r => r.map(cell => `"${String(cell).replace(/"/g,'""')}"`).join(',')).join('\n');
    downloadFile(`tabungku-riwayat-scan-${Utils.todayISO()}.csv`, '\uFEFF' + csv, 'text/csv;charset=utf-8');
    Utils.toast('Riwayat scan berhasil diexport ke CSV', 'success');
  }
  ...
  function exportScanPdf(){
    ...
    const rows = history.map(h => `
      <tr>
        <td>${Utils.escapeHtml(h.barcode)}</td>
        <td>${Utils.escapeHtml(h.name)}</td>
        <td style="text-align:right;">${Utils.formatRupiah(h.total || 0)}</td>
        <td>${new Date(h.scannedAt).toLocaleString('id-ID')}</td>
        <td>${Utils.escapeHtml(h.source || '-')}</td>
      </tr>
    `).join('');

    win.document.write(`
      <html><head><title>Riwayat Scan Barcode — TabungKu</title>
      <style>
        ...
        <h1>Riwayat Scan Barcode — TabungKu</h1>
        <p>Dicetak pada ${Utils.formatDateLong(Utils.todayISO())}</p>
        <div class="summary">
          <div>Total Scan: <b>${stats.totalScans}</b></div>
        </div>
        <table>
          <thead><tr><th>Barcode</th><th>Nama Produk</th><th>Total</th><th>Waktu Scan</th><th>Sumber</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>
