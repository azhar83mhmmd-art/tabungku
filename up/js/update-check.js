...
const UpdateCheck = (() => {
  ...
  const VERSION_JSON_URL = 'https://raw.githubusercontent.com/azhar83mhmmd-art/tabungku/main/version.json';
  ...
  async function check(){
    try{
      if (!navigator.onLine){ console.info('[UpdateCheck] Offline, skip cek update.'); return; }
      const res = await fetch(VERSION_JSON_URL + '?t=' + Date.now(), { cache: 'no-store' });
      if (!res.ok){ console.warn('[UpdateCheck] version.json gagal diambil, status:', res.status, VERSION_JSON_URL); return; }
      const info = await res.json();
      if (!info || !info.version){ console.warn('[UpdateCheck] version.json tidak valid:', info); return; }
      ...
      console.info('[UpdateCheck] versi lokal:', CURRENT_VERSION, '| versi repo:', info.version, '| lebih baru:', isNewer);

      if (isNewer && !alreadyNotified){
        showUpdatePopup(info);
      }
    }catch(e){
      console.warn('[UpdateCheck] Cek update gagal:', e.message, '— pastikan VERSION_JSON_URL benar dan version.json ada di repo.');
    }
  }
